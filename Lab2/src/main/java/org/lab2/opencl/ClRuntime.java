...
public final class ClRuntime implements AutoCloseable {
  ...
  public static final class Kernel implements Comparable<Kernel> {
    ...
    private void addArg(int index, float val) {
      try (MemoryStack stack = MemoryStack.stackPush()) {
        FloatBuffer fltBuff = stack.mallocFloat(1);
        fltBuff.put(val).flip();
        validateArg(clSetKernelArg(this.getId(), index, fltBuff));
      }
    }

    private void addArg(int index, double val) {
      try (MemoryStack stack = MemoryStack.stackPush()) {
        DoubleBuffer dblBuff = stack.mallocDouble(1);
        dblBuff.put(val).flip();
        validateArg(clSetKernelArg(this.getId(), index, dblBuff));
      }
    }
